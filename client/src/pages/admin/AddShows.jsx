  ...
  return nowPlayingMovies.length > 0 ? (
            ...
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={()=> setSelectedMovies(movie.id)}>
              ...
              {selectedMovies === movie.id && (
                <div className="absolute top-2 right-2 flex items-center justify-center bg-primary h-6 w-6 rounded">
                  <CheckIcon className="w-4 h-4 texxt-white" strokeWidth={2.5}/>
                </div>
              )} 
    ...
    <div className="mt-6">
      ...
      <div className="inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg">
        ...
        <button onClick={handleDataTimeAdd} className="bg-[#7f1d1d]/80 text-white px-3 py-2 text-sm rounded-lg hover:bg-[#991b1b] cursor-pointer" >
        Add Time
        </button>
      </div>
    </div>
...
<button className="bg-[#7f1d1d] text-white px-8 py-2 mt-6 rounded hover:bg-[#991b1b] transition-all cursor-pointer">
Add Show
</button>
      </>
  ) : <Loading/>
